...
var app = new Vue({
  ...
  created: function () {
    ...
    db.createIndex({ index: { fields: ["type"] } })
      ...
      .then((data) => {
        ...
        var blendedOutIds =
          JSON.parse(localStorage.getItem("blendedOutIds")) || [];

        // Filter out the items with ids that are in the blendedOutIds array
        var filteredItems = data.docs.filter(
          (item) => !blendedOutIds.includes(item._id)
        );
  ...
  methods: {
    ...
    onAddListItem: function () {
      ...
      if (!this.newItemQuantity || this.newItemQuantity < 1)
    ...
    /**
     * Display the map and place a marker
     */
    displayMap: function () {
      if (this.singleList.place.lat && this.singleList.place.lon) {
        var map = L.map("map").setView(
          [this.singleList.place.lat, this.singleList.place.lon],
          13
        );
        L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
          attribution: "© OpenStreetMap contributors",
        }).addTo(map);
        L.marker([this.singleList.place.lat, this.singleList.place.lon])
          .addTo(map)
          .bindPopup(this.singleList.place.title)
          .openPopup();
      } else {
